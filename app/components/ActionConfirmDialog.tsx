...
export function ActionConfirmDialog({ confirmation, actionsLeft, onCancel, onConfirm }: { confirmation: ActionConfirmation; actionsLeft: number; onCancel: () => void; onConfirm: () => void }) {
  return <AccessibleDialog onClose={onCancel} labelledBy="confirm-title" overlayClassName="confirm-overlay" dialogClassName="action-confirm-dialog">
    ...
    <footer><button type="button" className="dialog-secondary" onClick={onCancel}>選び直す</button><button type="button" className="primary" onClick={onConfirm}>Actionを使って実行する</button></footer>
  </AccessibleDialog>;
}
