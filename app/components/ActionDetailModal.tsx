...
export function ActionDetailModal({ action, actionsLeft, disabled, executeLabel, onClose, onExecute }: { action: PMActionDefinition; actionsLeft: number; disabled: boolean; executeLabel?: string; onClose: () => void; onExecute: () => void }) {
  return <AccessibleDialog onClose={onClose} labelledBy="action-detail-title" overlayClassName="action-detail-overlay" dialogClassName="action-detail-dialog">
    ...
    <footer><div className="modal-action-cost"><span>消費</span><strong>{action.id === "hearing" && !executeLabel ? "質問実行時 Action × 1" : "Action × 1"}</strong></div><div className="modal-action-remaining"><span>残りAction</span><strong>{actionsLeft}</strong><i>→</i><b>{Math.max(0, actionsLeft - 1)}</b></div><div className="modal-action-buttons"><button type="button" className="dialog-secondary" onClick={onClose}>戻る</button><button type="button" className="primary" disabled={disabled} onClick={onExecute}>{executeLabel ?? (action.id === "hearing" ? "話す相手を選ぶ" : action.title)}</button></div></footer>
  </AccessibleDialog>;
}
