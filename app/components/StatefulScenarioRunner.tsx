...
import type { ScenarioAction, ScenarioActionCategory, ScenarioDecision, SimulationMetrics, StatefulScenarioDefinition } from "@/src/data/statefulScenarioTypes";
...
const defaultActionCategories: ScenarioActionCategory[] = [
  { id: "hearing", label: "関係者ヒアリング", description: "誰に何を聞くか選ぶ", icon: "◉" },
  { id: "schedule", label: "スケジュール点検", description: "残作業と依存関係を見る", icon: "◷" },
  { id: "risk", label: "リスク整理", description: "不確実性を整理する", icon: "△" },
  { id: "scope", label: "要件・スコープ整理", description: "価値と範囲を整理する", icon: "◎" },
  { id: "team", label: "チーム状況確認", description: "負荷と余力を見る", icon: "♟" },
  { id: "report", label: "状況共有・報告", description: "必要な相手へ共有する", icon: "↗" },
];
...
export default function StatefulScenarioRunner({ scenario, difficulty, onExit }: { scenario: StatefulScenarioDefinition; difficulty: Difficulty; onExit: () => void }) {
  ...
  const [lastActionResult, setLastActionResult] = useState<{ action: ScenarioAction; unlocked: string[]; result: string; whyThisResult: string }>();
  ...
  const availableActionSpace = scenario.actions;
  const turnActions = availableActionSpace.filter((action) => action.availableFromTurn <= turnIndex + 1);
  ...
  const getActionKey = (action: ScenarioAction) => action.repeatPolicy === "per-turn" ? `${turnIndex + 1}:${action.id}` : `once:${action.id}`;
  const getActionAvailability = (action: ScenarioAction) => {
    ...
    if (usedActionKeys.includes(getActionKey(action))) return { disabled: true, label: action.repeatPolicy === "per-turn" ? "このターンで確認済み" : "確認済み" };
    return { disabled: false };
  };

  const executeAction = () => {
    if (!selectedAction || getActionAvailability(selectedAction).disabled) return;
    const turnOutcome = selectedAction.outcomesByTurn?.[turnIndex + 1];
    const grantedInformation = turnOutcome?.grantsInformation ?? selectedAction.grantsInformation;
    const unlocked = grantedInformation.filter((id) => !informationSet.has(id));
    setInformationIds((current) => [...new Set([...current, ...unlocked])]);
    setFlags((current) => mergeFlags(mergeFlags(current, selectedAction.setsFlags), turnOutcome?.setsFlags));
    setMetrics((current) => applyMetrics(current, selectedAction.metricEffects ?? {}));
    setInvestigationsLeft((value) => value - 1);
    if (selectedAction.repeatPolicy !== "always") setUsedActionKeys((current) => [...current, getActionKey(selectedAction)]);
    const result = turnOutcome?.result ?? selectedAction.result;
    const whyThisResult = turnOutcome?.whyThisResult ?? selectedAction.whyThisResult ?? "確認先と質問内容に応じた情報が得られました。";
    setLastActionResult({ action: selectedAction, unlocked, result, whyThisResult });
    setChain((current) => [...current, { turn: turnIndex + 1, timing: turn.timing, kind: "information", title: selectedAction.title, effect: unlocked.length ? unlocked.map((id) => scenario.information.find((info) => info.id === id)?.label).filter(Boolean).join("・") + "を把握" : result }]);
  ...
  const executeDecision = () => {
    ...
    setMetrics(after); setFlags(nextFlags); setDecisions((current) => [...current, record]); setLastDecision(record);
  ...
  const advanceTurn = () => {
    ...
    setMetrics(nextMetrics); setChain((current) => [...current, ...consequences]); setTurnIndex(nextIndex); setInvestigationsLeft(investigationBudget); setLastActionResult(undefined); setLastDecision(undefined); setPhase("situation");
  };
  ...
  if (phase === "final") {
    ...
    const sourceActionsByInformation = new Map(scenario.information.map((info) => [
      info.id,
      availableActionSpace.filter((action) => action.grantsInformation.includes(info.id) || Object.values(action.outcomesByTurn ?? {}).some((outcome) => outcome.grantsInformation?.includes(info.id))).map((action) => action.title),
    ]));
  ...
  return (
    <main className="stateful-shell">
      ...
      {phase === "action" && <section className="stateful-action-view">
        <div className="stateful-action-heading"><div><p className="v2-kicker">TURN {turnIndex + 1} — PLAYER ACTION</p><h1>次に何を確かめますか？</h1><span>{turn.title}</span></div><div className="stateful-action-budget"><div><span>INVESTIGATION</span><strong>{investigationsLeft}</strong><small>残り調査</small></div><div><span>DECISION</span><strong>1</strong><small>最終判断</small></div></div></div>
        <div className="stateful-play-grid"><div><h2>PMアクション</h2><ScenarioActionExplorer key={turn.id} categories={scenario.actionCategories ?? defaultActionCategories} actions={turnActions} stakeholders={scenario.stakeholders} difficulty={difficulty} relevantActionIds={relevantActionIds} getAvailability={getActionAvailability} onSelect={setSelectedAction} /></div><aside><h2>{difficulty === "guided" ? `判断材料 ${informationIds.length} / ${scenario.information.length}` : "判断材料"}</h2><div className={`stateful-info-list difficulty-${difficulty}`}>{difficulty === "guided" ? scenario.information.map((info) => informationSet.has(info.id) ? <div key={info.id} className="unlocked"><strong>✓ {info.label}</strong><span>{info.detail}</span></div> : <div key={info.id}><strong>🔒 未確認</strong><span>入手先の手がかり：{info.source}</span></div>) : scenario.information.filter((info) => informationSet.has(info.id)).map((info) => <div key={info.id} className="unlocked"><strong>✓ {info.label}</strong>{difficulty === "standard" ? <span>{info.detail}</span> : null}</div>)}</div>{difficulty !== "guided" && informationIds.length === 0 ? <p className="stateful-info-empty">取得済みの判断材料はありません。</p> : null}</aside></div>
        <div className="stateful-decision-entry"><div><strong>このターンの最終判断へ進む</strong><span>{investigationsLeft > 0 ? `調査枠はあと${investigationsLeft}回です。残したまま判断することもできます。` : "調査枠を使い切りました。"}{hiddenDecisionCount > 0 && " 得た情報によって選択肢が増える場合があります。"}</span></div><button onClick={() => visibleDecisions[0] && setSelectedDecision(visibleDecisions[0])}>判断案を見る →</button></div><button className="v2-text-button" onClick={() => setPhase("situation")}>← 状況を読み直す</button>
      </section>}

      {phase === "action-result" && lastActionResult && <section className="stateful-result"><p className="v2-kicker">ACTION RESULT</p><h1>{lastActionResult.action.title}</h1><article><span>WHAT HAPPENED</span><p>{lastActionResult.result}</p></article><article><span>WHY</span><p>{lastActionResult.whyThisResult}</p></article>{lastActionResult.unlocked.length > 0 && <article className="stateful-unlock"><span>NEW INFORMATION</span>{lastActionResult.unlocked.map((id) => { const info = scenario.information.find((item) => item.id)!; return <div key={id}><b>🔓 {info.label}</b><p>{info.detail}</p></div>})}</article>}<div className="stateful-result-actions">{investigationsLeft > 0 ? <button className="v2-secondary" onClick={() => setPhase("action")}>次の調査を選ぶ（残り{investigationsLeft}）</button> : null}<button className="primary" onClick={() => visibleDecisions[0] && setSelectedDecision(visibleDecisions[0])}>このターンの判断へ</button></div></section>}

      {selectedAction && <AccessibleModal onClose={() => setSelectedAction(undefined)} labelledBy="stateful-action-title" className="v2-choice-modal"><button className="v2-modal-close" onClick={() => setSelectedAction(undefined)} aria-label="閉じる">×</button><p className="v2-kicker">INVESTIGATION CHECK</p><h2 id="stateful-action-title">{selectedAction.question ?? selectedAction.title}</h2>{selectedAction.stakeholderId ? <p className="stateful-modal-target">確認先：{scenario.stakeholders.find((person) => person.id === selectedAction.stakeholderId)?.name}（{scenario.stakeholders.find((person) => person.id === selectedAction.stakeholderId)?.role}）</p> : null}<p>{selectedAction.description}</p><div><span>この行動の狙い</span><p>意思決定に必要な情報を増やします。プロジェクトの状態そのものが自動で回復するわけではありません。</p></div><p className="v2-decision-note">残り調査アクション {investigationsLeft} → {investigationsLeft - 1}</p><div className="v2-modal-actions"><button className="v2-secondary" onClick={() => setSelectedAction(undefined)}>戻る</button><button className="primary" onClick={executeAction}>調査アクションを実行</button></div></AccessibleModal>}

      {selectedDecision && <AccessibleModal onClose={() => setSelectedDecision(undefined)} labelledBy="stateful-decision-title" className="stateful-decision-modal"><button className="v2-modal-close" onClick={() => setSelectedDecision(undefined)} aria-label="閉じる">×</button><p className="v2-kicker">TURN DECISION</p><h2 id="stateful-decision-title">PMとして、どう判断しますか？</h2><p>{turn.thinkingPoint}</p><div className="stateful-decision-options">{visibleDecisions.map((decision) => { const selected = selectedDecision.id === decision.id; return <button key={decision.id} className={selected ? "selected" : ""} onClick={() => setSelectedDecision(decision)}><strong>{decision.title}{decision.irreversible && <em>正式な判断</em>}</strong><span>{difficulty === "challenge" ? "この判断を選択肢として検討します。" : decision.description}</span></button>})}</div><div className="stateful-selected-decision"><span>選択中</span><strong>{selectedDecision.title}</strong><p>{selectedDecision.description}</p><small>このターンの最終判断：1回</small></div><div className="v2-modal-actions"><button className="v2-secondary" onClick={() => setSelectedDecision(undefined)}>戻る</button><button className="primary" onClick={executeDecision}>この判断を確定する</button></div></AccessibleModal>}
